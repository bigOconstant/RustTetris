...
impl Level {
    ...
    pub fn draw_level(&self,canvas: &mut sdl2::render::WindowCanvas,level_string:String) {
        ...
        let target = Rect::new((dimentions.left - dimentions.unit_size *6) as i32,dimentions.top - 10,width/3 as u32,height/3 as u32);
    ...
    pub fn draw_score(&self,canvas: &mut sdl2::render::WindowCanvas,score:i32) {
        ...
        let TextureQuery { width, height, .. } = texture.query();
        let target = Rect::new((dimentions.left - dimentions.unit_size *6) as i32,dimentions.top+dimentions.unit_size * 2,width/3 as u32,height/3 as u32);
        canvas.set_draw_color(yellow);
        canvas.copy(&texture, None, Some(target)).unwrap();
        // End top half
       ...
       // let mut number_target = Rect::new((dimentions.left - dimentions.unit_size *6) as i32,dimentions.top+dimentions.unit_size * 5,width/3 as u32,height/2 as u32);
        let number_target = Rect::new((dimentions.left - dimentions.unit_size *6) as i32,dimentions.top+dimentions.unit_size * 4,width/3 as u32,height/3 as u32);
        canvas.set_draw_color(yellow);
